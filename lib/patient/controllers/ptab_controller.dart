...
class PatientTabController extends GetxController {
  ...
  Future<void> _startCallServicesIfLoggedIn() async {
    ...
    if (!isPatientLoggedIn || isDoctorLoggedIn) {
      debugPrint(
        "PATIENT_CALL_INIT_SKIPPED_ROLE :: patient=$isPatientLoggedIn doctor=$isDoctorLoggedIn",
      );
      return;
    }
    ...
    if (value == null) {
      debugPrint("PATIENT_CALL_INIT_SKIPPED :: Cube user not found");
      return;
    }

    _callServicesStarted = true;

    debugPrint("PATIENT_CALL_INIT_STARTED");
    checkSystemAlertWindowPermission();

    if (Get.context != null) {
      CallManager.instance.init(Get.context!);
    } else {
      debugPrint("PATIENT_CALL_INIT_WARNING :: Get.context is null");
    }

    await PushNotificationsManager.instance.init();

    await CallPushRouter.start(owner: 'patient');
    ...
    debugPrint("PATIENT_CALL_INIT_DONE");
  }
  // callNotification() async {
  //   FirebaseMessaging.onMessage.listen((message) {
  //     debugPrint('PATIENT_FOREGROUND_FCM_DATA :: ${message.data}');
  ...
  //     if (isStartCall) {
  //       debugPrint('PATIENT_FOREGROUND_CALL_PUSH_RECEIVED :: ${message.data}');
  ...
  //       } catch (e) {
  //         debugPrint("PATIENT_REJECT_CALL_ERROR :: $e");
  //       }
  ...
  //       } catch (e) {
  //         debugPrint("PATIENT_CALL_REJECT_PAYLOAD_ERROR :: $e");
  //       }
  ...
  //     debugPrint("PATIENT_INITIAL_FCM_DATA :: ${initialMessage.data}");
  ...
  //     debugPrint("PATIENT_OPENED_FCM_DATA :: ${message.data}");
